...
{
	explicit android_lifecycle_callbacks(xrt_instance *xinst) : instance(xinst) {}
	xrt_instance *instance;
	u_generic_callbacks<xrt_android_lifecycle_event_handler_t, enum xrt_android_lifecycle_event>
	    callback_collection;
};
...
int
android_lifecycle_callbacks_invoke(struct android_lifecycle_callbacks *alc,
	...
	try {
		return alc->callback_collection.invokeCallbacks(
		    event, [=](enum xrt_android_lifecycle_event event, xrt_android_lifecycle_event_handler_t callback,
		               void *userdata) { return callback(alc->instance, window, event, userdata); });
	}
	CATCH_CLAUSES("invoking callbacks", -1)
}

struct android_lifecycle_callbacks *
android_lifecycle_callbacks_create(struct xrt_instance *xinst)
{
	try {
		auto ret = std::make_unique<android_lifecycle_callbacks>(xinst);
