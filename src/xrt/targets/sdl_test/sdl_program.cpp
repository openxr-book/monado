...
sdl_program_plus_create()
{
	...
	// Init sub components.
	// The SDL window will be created later, during the start of the session
	sdl_instance_init(&spp);
	sdl_device_init(&spp);
	sdl_system_devices_init(&spp);
	sdl_compositor_init(&spp);
...
sdl_program_plus_render(struct sdl_program_plus *spp_ptr)
{
	...
	if (spp.c.base.slot.layer_count == 0) {
		...
		auto &ssc = *(sdl_swapchain *)l.sc_array[0];  // 0: left swapchain
...
sdl_create_window(struct sdl_program *sp, int width, int height)
{
	...
	sp->win = SDL_CreateWindow(title, x, y, width, height, window_flags);
...
{
	if (sp->ctx != NULL) {
		SDL_GL_DeleteContext(sp->ctx);
		sp->ctx = NULL;
	}
	if (sp->win != NULL) {
		SDL_DestroyWindow(sp->win);
		sp->win = NULL;
	}
}